...
typedef struct {
   ...
   int8_t resize_speed;
   uint8_t divider; // ticks to wait between updates (0 = every tick); floating point speeds are tempting
   uint8_t divider_tick; // low 8 bits of system_tick at last divided update
} motion_t;
...
void animate(uint16_t system_tick, symbol_t* symbols, uint8_t symbol_count) {
   ...
   for (uint8_t i=0; i<MIN(symbol_count,MAX_SYMBOLS); i++) {
      ...
      if ( sym->visible ) {
         motion_t *m = &motion[ i ];
         if ( m->divider ) {
            uint8_t elapsed = (uint8_t)system_tick - m->divider_tick;
            if ( elapsed < m->divider ) continue;
            m->divider_tick = (uint8_t)system_tick;
         }
...
void setRotationSpeed( uint8_t sid, int8_t rotation_speed, uint8_t rotation_speed_divider ) {
   motion_t *m = &motion[ sid ];
   m->divider = rotation_speed_divider;
