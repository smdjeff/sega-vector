...
// ferengi - angry
   ...
   0x80, 0x00, 0x00, 0x00,
     ...
     #define V_SMOKE (V_EXPLODE1+4)
      0x2A|SEGA_LAST, 0x42, 0x22, 0x03,
     #define V_SMOKE_LEN 1
...
void drawBonus( uint8_t value, bool reset ) {
   ...
   if ( score != last_value ) {
      last_value = value;
      if ( !reset ) {
         // when parameter changed assume it counted DOWN by one
         if (--d2 < '0') {
            d2 = '9';
            if (--d1 < '0') {
               d1 = '9';
